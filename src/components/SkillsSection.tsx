...
/* DE stack ordered by data flow layer */
const LAYERS = [
  {
    id: 'ingest',
    layer: '01 · Ingest',
...
function LevelBar({ level, color }: { level: number; color: string }) {
  return (
    <div className="flex gap-1 items-center">
      ...
      <span className="font-mono text-[9px] ml-1.5" style={{ color: 'var(--fg-35)' }}>
        {LEVEL_LABELS[level]}
      </span>
    </div>
  )
...
export default function SkillsSection() {
  return (
    <section className="py-12 sm:py-16 px-4 sm:px-6" style={{ background: 'var(--bg-surface)' }}>
      <div className="max-w-[1200px] mx-auto">
        ...
        <motion.h2
          ...
          className="font-sans text-[26px] md:text-[34px] leading-[1.15] tracking-tight mb-12"
          style={{ color: 'var(--fg)' }}
        >
          Every layer of the{' '}
          <span className="font-serif">data pipeline.</span>
        </motion.h2>

        {/* Stack layers — vertical list */}
        <div className="space-y-3">
          {LAYERS.map((layer, li) => (
            ...
            >
              ...
              {/* Skills grid */}
              <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-0 divide-x divide-y" style={{ borderColor: 'var(--border)' }}>
                {layer.skills.map((skill) => (
                  <div
                    key={skill.name}
                    className="flex flex-col gap-2 px-4 sm:px-5 py-4 transition-colors duration-150"
                    style={{ borderColor: 'var(--border)' }}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-medium text-sm leading-tight" style={{ color: 'var(--fg)' }}>
                        {skill.name}
                      </span>
                    </div>
        ...
        {/* Flow arrow between layers — decorative */}
        <motion.div
          initial={{ opacity: 0 }}
        ...
        >
          ...
          <span className="font-mono text-[9px] tracking-widest" style={{ color: 'var(--fg-30)' }}>
